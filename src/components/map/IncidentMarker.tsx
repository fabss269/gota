...
// desagüe = marrón. La prioridad se refleja como un anillo titilante
// alrededor del pin (rediseño 2026-08-12, pedido de Edgar — la versión
// bicolor con degradé no convenció; se vuelve al anillo pero ahora con las
// 3 prioridades del semáforo, no solo alerta/critica).
const CATEGORIA_COLOR: Record<IncidentCluster['categoriaDominante'], string> = {
  agua: Colors.agua,
  desague: Colors.desague,
};

// Color del anillo por prioridad — verde/amarillo/rojo. `a_tiempo` también
// titila (en verde) para que se lea el estado sin ambigüedad.
const PRIORIDAD_ANILLO: Record<IncidentCluster['prioridadMaxima'], string> = {
  ...
  return (
    <View style={styles.wrapper}>
      <Svg width={PIN_SIZE} height={PIN_SIZE * 1.25} viewBox="0 0 40 50">
        ...
        {/* Cast a `any` — PathProps no tipa `style.animation` (es CSS puro que
            solo aplica en web; en nativo la animación se ignora). */}
        <Path
          d={PIN_PATH}
          fill="none"
          stroke={anilloColor}
          strokeWidth={3}
          {...({ style: { animation: 'gota-pin-pulse 1.4s ease-in-out infinite' } } as unknown as Record<string, unknown>)}
        />
      </Svg>
